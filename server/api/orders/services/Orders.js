...
module.exports = {
   ...
   */
	...
	fetch: (params) => {
		...
		return Orders.findOne(_.pick(params, _.keys(Orders.schema.paths))).populate(populate);
	},
   ...
   */
	...
	count: (params) => {
		...
		return Orders.count().where(filters.where);
	},
   ...
   */
	...
	remove: async (params) => {
		...
		const data = await Orders.findOneAndRemove(params, {}).populate(populate);
